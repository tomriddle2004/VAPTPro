...
import {
  GitCompare, ArrowRight, ArrowLeft, Minus, Plus, RefreshCw, FileDown,
} from 'lucide-react';
...
import { generateComparisonPDF } from '@/lib/comparePDF';
...
function computeDiff(scanA: ScanDetail, scanB: ScanDetail): DiffResult {
  ...
  const common: FindingChange[] = scanA.findings
    ...
    .map(fA => {
      ...
      return { fA, fB, cvssChange: (fB.cvss_score ?? 0) - (fA.cvss_score ?? 0), severityChanged: fA.severity !== fB.severity };
    });
...
function ScanSelector({
  ...
  label: string; icon: typeof ArrowLeft; accent: string;
  scans: Scan[]; selectedId: string; onChange: (id: string) => void;
}) {
  return (
    <div>
      ...
      <select
        ...
        className="w-full border rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
...
function CvssChange({ change }: { change: number }) {
  if (change === 0) return <span style={{ color: 'var(--text-faint)' }} className="text-xs">—</span>;
  const up = change > 0;
  return (
...
function FindingRow({ finding, variant, cvssChange }: {
  finding: Finding; variant: 'new' | 'resolved' | 'changed' | 'unchanged'; cvssChange?: number;
}) {
  const bgMap = {
...
export default function Compare() {
  ...
  useEffect(() => {
    ...
    if (a && b) { setScanA(a); setScanB(b); setDiff(computeDiff(a, b)); }
    setLoading(false);
  }, [idA, idB]);

  const handleExportPDF = async () => {
    if (!scanA || !scanB || !diff) return;
    setExporting(true);
    await generateComparisonPDF(scanA, scanB, diff);
    setExporting(false);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            Side-by-side diff — new vulnerabilities, resolved issues, CVSS score drift
          </p>
        </div>
        ...
        {diff && scanA && scanB && (
          <button
            ...
            disabled={exporting}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/30 text-cyan-400 transition-colors text-sm disabled:opacity-60"
          >
            {exporting
              ? <><div className="w-4 h-4 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />Generating…</>
      ...
      {/* Scan selectors */}
      <div
        className="border rounded-xl p-5 grid sm:grid-cols-[1fr_auto_1fr] gap-4 items-end"
      ...
      >
        ...
        <div className="flex items-center justify-center pb-1">
          <GitCompare className="w-6 h-6" style={{ color: 'var(--text-faint)' }} />
        </div>
          ...
          {[
            ...
            <div key={scan.id} className={`border rounded-xl p-4 ${bg}`}>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span
                  className="text-xs px-2 py-0.5 border rounded capitalize font-mono"
      ...
      {diff && (
        ...
        >
          {/* Column headers */}
          <div
            className="grid grid-cols-[2rem_5rem_6rem_1fr_5rem_5rem] gap-2 px-4 py-2.5 border-b text-[10px] font-mono"
            style={{ backgroundColor: 'var(--bg-input)', borderBottomColor: 'var(--border-subtle)', color: 'var(--text-faint)' }}
          >
            <span></span><span>PORT</span><span>SERVICE</span><span>VERSION</span><span>SEVERITY</span><span className="text-center">CVSS Δ</span>
          </div>
          ...
          <div className="divide-y" style={{ '--tw-divide-color': 'var(--border-subtle)' } as React.CSSProperties}>
            ...
            {diff.changed.length > 0 && (
              <div>
                <div className="px-4 py-1.5 bg-yellow-500/5 flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-yellow-400">CVSS / SEVERITY CHANGED ({diff.changed.length})</span>
                </div>
            ...
            {!diff.newInB.length && !diff.resolvedInA.length && !diff.changed.length && !diff.unchanged.length && (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Both scans have no findings to compare.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {!diff && !loading && completedScans.length < 2 && (
        <div
          className="border rounded-xl p-8 flex flex-col items-center justify-center gap-3 text-center"
          style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}
        >
          ...
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
            Run assessments against different targets or the same target at different times to compare results.
          </p>
        </div>
      )}
    </div>
